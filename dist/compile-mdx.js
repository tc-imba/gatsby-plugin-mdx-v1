...
async function compileMDX({
  ...
  try {
    ...
    const processor = createProcessor(options); // Pass required custom data into the processor

    processor.data(`mdxNodeId`, await cache.get((0, _cacheHelpers.createFileToMdxCacheKey)(absolutePath)));
    processor.data(`mdxMetadata`, {});
    const result = await processor.process( // Inject path to original file for remark plugins. See: https://github.com/gatsbyjs/gatsby/issues/26914
    new VFile({
      value: source,
      path: absolutePath
...
const compileMDXWithCustomOptions = async ({
  ...
  const customPluginOptions = (0, _deepmerge.default)(Object.assign({}, pluginOptions), customOptions); // Prepare MDX compile
